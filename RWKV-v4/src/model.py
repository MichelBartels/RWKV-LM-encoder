...
class RWKV_TimeMix(nn.Module):
    ...
    def forward(self, x):
        ...
        rwkv = torch.sigmoid(r) * RUN_CUDA(B, T, C // 2, self.time_decay, self.time_first, k, v)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.layer_id == 0 and self.config.model_type == 'RWKV-ffnPre':
            ...
            x = x + torch.cat([self.ffnPre_forward(normalized), self.ffnPre_backward(torch.flip(normalized, [-1]))], -1)  # better in some cases
        else:
            normalized = self.ln1(x)
